...
public strictfp class RobotPlayer {
     ...
     * ,     23     ,      24     ,    25-28   ,       29-46       ,       47-63       ]
     *  rewriteEhQ's    hqsActed   comeKillDude      adamWells           ManaWells
     */
    ...
    //not using these island ones rn
    static final int islandLocsIndex = 29;
    static final int islandLocsLastIndex = 41;
    static final int wellLocsIndex = 29;
    static final int adamWellLocsIndex = 29;
    static final int adamWellLocsLastIndex = 46;
    static final int manaWellLocsIndex = 47;
    static final int manaWellLocsLastIndex = 63;
    static final int wellLocsLastIndex = 63;

//    static ArrayList<WellInfo> wellsThisRobotHasSeen = new ArrayList<>(wellLocsLastIndex-wellLocsIndex);
    static HashSet<MapLocation> adamWellsThisRobotHasSeen = new HashSet<MapLocation>(adamWellLocsLastIndex-adamWellLocsIndex);
    static HashSet<MapLocation> manaWellsThisRobotHasSeen = new HashSet<MapLocation>(manaWellLocsLastIndex-manaWellLocsIndex);
    ...
    static void runCarrier(RobotController rc) throws GameActionException {
        ...
        MapLocation aHqPos = getTheClosestHq(rc);
        if (aHqPos.isWithinDistanceSquared(me, GameConstants.DISTANCE_SQUARED_FROM_HEADQUARTER)) {
            updateWellsInArray(rc);
            //printSharedArray(rc);
        } else {
            anyWellsCheck(rc);
        }
        ...
        for (int yeaboiiiiiii = 1; yeaboiiiiiii <= 2; yeaboiiiiiii++) {
            ...
            if (carryingAnAnchor) {
                ...
                if ((amountOfAdamantium + amountOfMana) < 40) {
                    ...
                    ArrayList<MapLocation> adamWells = new ArrayList<MapLocation>();
                    ArrayList<MapLocation> manaWells = new ArrayList<MapLocation>();

                    for (int i = 0; i < (adamWellLocsLastIndex - adamWellLocsIndex); i++) {
                        int thingAtIndex = rc.readSharedArray(adamWellLocsIndex + i);
                        if (thingAtIndex != 0) {
                            MapLocation adamWell = intToLocation(rc, rc.readSharedArray(adamWellLocsIndex + i));
                            adamWells.add(adamWell);
                        }
                        thingAtIndex = rc.readSharedArray(manaWellLocsIndex + i);
                        if (thingAtIndex != 0) {
                            MapLocation manaWell = intToLocation(rc, rc.readSharedArray(manaWellLocsIndex + i));
                            manaWells.add(manaWell);
                        }
                    }
                    ...
                    if (wells.size() > 0) {
                        MapLocation closestWellLoc = wells.get(0);

                        for (MapLocation wellLoc : wells) {
                            if (me.distanceSquaredTo(wellLoc) < me.distanceSquaredTo(closestWellLoc)) {
                                closestWellLoc = wellLoc;
                            }
                        }
                        rc.setIndicatorString(String.valueOf(closestWellLoc));
                        ...
                        if (me.isAdjacentTo(closestWellLoc)) {
                            ...
                            if (rc.isActionReady()) {
                                if (rc.canCollectResource(closestWellLoc, -1)) {
...
//        moveDirs[4] = dir.rotateLeft().rotateLeft();
    ...
    static void anyWellsCheck(RobotController rc) throws GameActionException {
        WellInfo[] wellsWeCanSee = rc.senseNearbyWells();
        for (WellInfo wellInfo : wellsWeCanSee) {
            if (wellInfo.getResourceType() == ResourceType.ADAMANTIUM) {
                adamWellsThisRobotHasSeen.add(wellInfo.getMapLocation());
            } else {
                manaWellsThisRobotHasSeen.add(wellInfo.getMapLocation());
            }
        }
//        int rangeOfWellsInArray = rc.readSharedArray(wellLocsIndex) - rc.readSharedArray(wellLocsLastIndex);
//        ArrayList<MapLocation> wellsAlreadyInArray = new ArrayList<>(rangeOfWellsInArray);
////        MapLocation[] wellsAlreadyInArray = new MapLocation[rangeOfWellsInArray];
//        for (int i = 0; i < rangeOfWellsInArray; i++) {
//            wellsAlreadyInArray.add(intToLocation(rc, rc.readSharedArray(wellLocsIndex + i)));
//        }
//        for (WellInfo aWellWeCanSee : wellsWeCanSee) {
//            if (!(wellsAlreadyInArray.contains(aWellWeCanSee))) {
//
//            }
//        }
    }

    static void updateWellsInArray(RobotController rc) throws GameActionException {
        int rangeOfAdamWellsInArray = adamWellLocsLastIndex - adamWellLocsIndex;
        int rangeOfManaWellsInArray = manaWellLocsLastIndex - manaWellLocsIndex;
//        ArrayList<MapLocation> wellsAlreadyInArray = new ArrayList<>(rangeOfWellsInArray);
//        MapLocation[] adamWellsAlreadyInArray = new MapLocation[rangeOfWellsInArray/2];
//        MapLocation[] manaWellsAlreadyInArray = new MapLocation[rangeOfWellsInArray/2];

        HashSet<MapLocation> adamWellsAlreadyInArray = new HashSet<>(rangeOfAdamWellsInArray);
        HashSet<MapLocation> manaWellsAlreadyInArray = new HashSet<>(rangeOfManaWellsInArray);

        for (int i = 0; i < (rangeOfAdamWellsInArray); i++) {
            adamWellsAlreadyInArray.add(intToLocation(rc, rc.readSharedArray(adamWellLocsIndex + i)));
            manaWellsAlreadyInArray.add(intToLocation(rc, rc.readSharedArray(manaWellLocsIndex + i)));
        }

        adamWellsAlreadyInArray.addAll(adamWellsThisRobotHasSeen);
        manaWellsAlreadyInArray.addAll(manaWellsThisRobotHasSeen);

        int i = 0;
        for (MapLocation adamWellToAdd : adamWellsAlreadyInArray) {
            rc.writeSharedArray(adamWellLocsIndex + i, locationToInt(rc, adamWellToAdd));
            i++;
        }
        i = 0;
        for (MapLocation manaWellToAdd : manaWellsAlreadyInArray) {
            rc.writeSharedArray(manaWellLocsIndex + i, locationToInt(rc, manaWellToAdd));
            i++;
        }
    }
...
//    static void anyIslandsCheck(RobotController rc) {
//        int[] islandIds = rc.senseNearbyIslands();
//        for (int islandId : islandIds) {
//            rc.senseNearbyIslandLocations(islandId)
//        }
//    }
